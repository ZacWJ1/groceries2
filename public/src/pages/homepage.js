...
const HomePage = () => {
  const [items, setItems] = useState([]); const [grocers, setGroceries] = useState([]); const location = useLocation(); const navigate = useNavigate(); const [user, setUser] = useState(location.state?.user); const [loading, setLoading] = useState(true); const fetchGrocers = async () => { const res = await axios.get('http://localhost:4444/groceries'); setGroceries(res.data); }; useEffect(() => { const fetchUser = async () => { try { const response = await axios.get('http://localhost:4444/user', { withCredentials: true }); if (response.data.user) { setUser(response.data.user); } else { navigate('/login'); } } catch (error) { navigate('/login'); } }; const fetchItems = async () => { const res = await axios.get('http://localhost:4444/items'); setItems(res.data); }; const fetchData = async () => { if (!user) { await fetchUser(); } await fetchItems(); await fetchGrocers(); setLoading(false); }; fetchData(); }, [user, navigate]); if (loading) { return <center><h1>Loading...</h1></center>; } const handleDelete = async (id) => { try { await axios.delete(`http://localhost:4444/items/${id}`); setItems(items.filter((food) => food._id !== id)); } catch (error) { console.error('Error deleting item', error); } }; const handleGroceryDelete = async (id) => { try { await axios.delete(`http://localhost:4444/groceries/${id}`); setGroceries(grocers.filter((grocery) => grocery._id !== id)); // Refresh grocers
